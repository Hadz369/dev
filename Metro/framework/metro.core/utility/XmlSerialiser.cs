                                    ...
                                    foreach (DataColumn c in dt.Columns)
                                    {
                                        xtw.WriteStartElement("cell");
                                        {
                                            ...
                                            if (r[c].GetType() == typeof(DateTime))
                                                val = ((DateTime)r[c]).ToString("yyyy-MM-ddTHH:mm:ss.ffffff");
                                            else
                                                val = r[c].ToString();

                                            xtw.WriteAttributeString("ix", (cx++).ToString());
                                            xtw.WriteAttributeString("value", val);
        ...
        */
        ...
        private static DataTable DataTableFromXmlElement(XmlElement element)
        {
            ...
            if (element.Name == "datatable")
            {
                ...
                XmlNodeList columns = element.GetElementsByTagName("cols");
                if (columns.Count > 0)
                {
        ...
        public static String PrepareXml(String xml)
        {
            ...
            using (MemoryStream mStream = new MemoryStream())
            {
                using (XmlTextWriter writer = new XmlTextWriter(mStream, Encoding.Unicode))
                {
                    ...
                    {
                        throw new Exception("Error reformatting XML", ex);
                    }
