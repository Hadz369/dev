...
namespace HS
{
    ...
    public class ChannelBroker<T> : IDisposable
    {
        ...
        Dictionary<string, DuplexChannelFactory<T>> _registered = new Dictionary<string, DuplexChannelFactory<T>>();
        ...
        {
            return _registered.ContainsKey(name.ToUpper());
        }
        ...
        public bool Register(string key, DuplexChannelFactory<T> factory)
        {
        ...
        public ChannelProxy<T> GetProxy(string key)
        {
            ...
            {
                var p = new ChannelProxy<T>((_registered[key]).CreateChannel());
                //(p.Channel as ICommunicationObject).Open();
                return p;
            }
            else
            {
        ...
        {
            foreach (KeyValuePair<string, DuplexChannelFactory<T>> kvp in _registered)
            {
                SendMessage(TraceLevel.Info, String.Concat("Registered Channel: Key=", kvp.Key, ", Uri=", kvp.Value.Endpoint.ListenUri.ToString()), null);
            }
        }
